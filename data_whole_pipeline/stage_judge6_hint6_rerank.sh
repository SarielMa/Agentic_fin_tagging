...
VERDICTS="${RUN}/verdicts_k10_judge6_hint6/llm_verifier_verdicts.json"
ARM=no_determ
OUT="${RUN}/rerank_no_determ_k10judge6_hint6"
...
if [[ ! -f "${RANKING}" ]]; then
  echo "[$(date +%H:%M:%S)] dumping judge6_hint6 arm (all 6 dims judged AND supplied as hint) (mode=llm_drop) against verdicts_k10_judge6_hint6"
...
sbatch --job-name="rr_judge6h6_k10" \
