...
RUN_DIR="${REPO_ROOT}/runs_ags_verifier_ablation/qwen3_32b"
...
python "${REPO_ROOT}/run_ags_verifier_ablation.py" --output-dir "${RUN_DIR}"
rc=$?
if [[ ${rc} -ne 0 ]]; then
...
python "${REPO_ROOT}/build_verifier_ablation_table.py" --run-dir "${RUN_DIR}"
echo "[$(date +%T)] done"
