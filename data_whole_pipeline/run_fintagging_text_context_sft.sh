...
# End-to-end SFT pipeline for FinTagging text sentence-context extraction.
#
# Task:
#   financial text -> JSON list of
#   {"numeric_entity", "datatype", "sentence_context"}

REPO_ROOT="$(readlink -f "$(dirname "$0")")"

DATASET_PATH="${DATASET_PATH:-${REPO_ROOT}/FinTagging_800_200_text_context_sft_arrow}"
...
PIPELINE_ROOT="${PIPELINE_ROOT:-${REPO_ROOT}/runs_fintagging_text_context}"
...
require_path "${DATASET_PATH}" "text context SFT dataset"
require_path "${REPO_ROOT}/sft_value_type_peft_ddp.py" "SFT script"
require_path "${REPO_ROOT}/merge_lora_value_type.py" "merge script"
require_path "${REPO_ROOT}/generate_fintagging_value_type_predictions.py" "generation script"
require_path "${REPO_ROOT}/evaluate_fintagging_context_extraction.py" "context extraction evaluator"
...
echo "============================================================"
echo "TASK         : text_context_extraction"
...
if [[ "${TRAIN}" == "1" ]]; then
  torchrun --nproc_per_node="${TP}" "${REPO_ROOT}/sft_value_type_peft_ddp.py" \
...
if [[ "${MERGE}" == "1" ]]; then
  ...
  python "${REPO_ROOT}/merge_lora_value_type.py" \
    --base "${MODEL}" \
...
if [[ "${PREDICT}" == "1" ]]; then
  ...
  python "${REPO_ROOT}/generate_fintagging_value_type_predictions.py" \
    --model "${MERGED_DIR}" \
...
if [[ "${EVAL}" == "1" ]]; then
  ...
  python "${REPO_ROOT}/evaluate_fintagging_context_extraction.py" \
