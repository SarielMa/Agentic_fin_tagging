...
BACKUP="${REPO_ROOT}/runs_ags_verifier_bridge/qwen3_32b/blocked_backup"
mkdir -p "${BACKUP}"
for f in bridge_summary.json final_reranker_comparison.csv bridge_table.tex; do
  src="${REPO_ROOT}/runs_ags_verifier_bridge/qwen3_32b/${f}"
  [[ -f "${src}" && ! -f "${BACKUP}/${f}" ]] && cp "${src}" "${BACKUP}/${f}"
done

EMIT_LATEX=1 "${REPO_ROOT}/run_ags_verifier_bridge.sh"
echo "[$(date +%T)] done"
