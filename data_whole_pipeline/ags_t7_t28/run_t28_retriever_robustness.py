...
(sum-RRF fusion, `agree()`, the consensus rerank, beta) is `ags_table5_ablation.core.evaluate`
...
2. beta is not re-tuned. The rerank operates on range-normalized fused scores, so beta
   ...
   spec asks for it to be stated in the caption.
    ...
    python ags_t7_t28/run_t28_retriever_robustness.py --limit 25   # smoke test
    python ags_t7_t28/run_t28_retriever_robustness.py              # full grid
"""
...
def main() -> None:
    ...
    metrics = {
        ...
        "consolidation": (
            "ags_table5_ablation.core.evaluate at the frozen AGS config (sum-RRF, "
