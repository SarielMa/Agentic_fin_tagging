#!/usr/bin/env bash
set -euo pipefail

# End-to-end SFT pipeline for FinTagging table context extraction.
#
# Task:
#   HTML table -> JSON list of
#   {"numeric_entity", "datatype", "row_context", "column_context"}

REPO_ROOT="$(readlink -f "$(dirname "$0")")"

DATASET_PATH="${DATASET_PATH:-${REPO_ROOT}/FinTagging_800_200_table_context_sft_arrow}"
DEFAULT_MODELS="meta-llama/Llama-3.3-70B-Instruct"
if [[ -n "${MODEL:-}" ]]; then
  MODELS="${MODEL}"
else
  MODELS="${MODELS:-${DEFAULT_MODELS}}"
fi

TP="${TP:-1}"
MAX_LEN="${MAX_LEN:-16384}"
BATCH_SIZE="${BATCH_SIZE:-1}"
GRAD_ACCUM="${GRAD_ACCUM:-8}"
EPOCHS="${EPOCHS:-3}"
LR="${LR:-2e-4}"
BF16="${BF16:-1}"
USE_QLORA="${USE_QLORA:-1}"
MAX_NEW_TOKENS="${MAX_NEW_TOKENS:-4096}"

TRAIN="${TRAIN:-1}"
MERGE="${MERGE:-1}"
PREDICT="${PREDICT:-1}"
EVAL="${EVAL:-1}"

PIPELINE_ROOT="${PIPELINE_ROOT:-${REPO_ROOT}/runs_fintagging_table_context}"

model_slug () {
  local model_basename="$1"
  model_basename="${model_basename,,}"
  model_basename="${model_basename/llama-/llama}"
  model_basename="${model_basename//-/_}"
  printf '%s' "${model_basename}"
}

require_path () {
  local path="$1"
  local label="$2"
  if [[ ! -e "${path}" ]]; then
    echo "Missing ${label}: ${path}" >&2
    exit 1
  fi
}

export TOKENIZERS_PARALLELISM=false
export NCCL_ASYNC_ERROR_HANDLING=1

if [[ -z "${HF_TOKEN:-}" && -f "${HOME}/.cache/huggingface/token" ]]; then
  export HF_TOKEN="$(cat "${HOME}/.cache/huggingface/token")"
fi

require_path "${DATASET_PATH}" "table context SFT dataset"
require_path "${REPO_ROOT}/sft_value_type_peft_ddp.py" "SFT script"
require_path "${REPO_ROOT}/merge_lora_value_type.py" "merge script"
require_path "${REPO_ROOT}/generate_fintagging_value_type_predictions.py" "generation script"
require_path "${REPO_ROOT}/evaluate_fintagging_context_extraction.py" "context extraction evaluator"

mkdir -p "${PIPELINE_ROOT}"
PIPELINE_ROOT="$(readlink -f "${PIPELINE_ROOT}")"

for MODEL in ${MODELS}; do
MODEL_TAG="$(basename "${MODEL}")"
MODEL_SLUG="$(model_slug "${MODEL_TAG}")"
RUN_TAG="sft_${EPOCHS}ep"

MODEL_DIR="${PIPELINE_ROOT}/${MODEL_SLUG}/${RUN_TAG}"
ADAPTER_OUT="${MODEL_DIR}/sft_adapter"
ADAPTER_PATH="${ADAPTER_OUT}/lora_adapter"
MERGED_DIR="${MODEL_DIR}/merged"
PRED_DIR="${MODEL_DIR}/predictions"
EVAL_DIR="${MODEL_DIR}/eval"
LOG_DIR="${MODEL_DIR}/logs"

mkdir -p "${ADAPTER_OUT}" "${MERGED_DIR}" "${PRED_DIR}" "${EVAL_DIR}" "${LOG_DIR}"

echo "============================================================"
echo "TASK         : table_context_extraction"
echo "DATASET_PATH : ${DATASET_PATH}"
echo "MODELS       : ${MODELS}"
echo "MODEL        : ${MODEL}"
echo "MODEL_DIR    : ${MODEL_DIR}"
echo "TP           : ${TP}"
echo "MAX_LEN      : ${MAX_LEN}"
echo "MAX_NEW_TOKENS: ${MAX_NEW_TOKENS}"
echo "EPOCHS       : ${EPOCHS}"
echo "LR           : ${LR}"
echo "USE_QLORA    : ${USE_QLORA}"
echo "BF16         : ${BF16}"
echo "============================================================"

BF16_FLAG=()
if [[ "${BF16}" == "1" ]]; then
  BF16_FLAG=(--bf16)
fi

QLORA_FLAG=()
if [[ "${USE_QLORA}" == "1" ]]; then
  QLORA_FLAG=(--use_qlora)
fi

if [[ "${TRAIN}" == "1" ]]; then
  torchrun --nproc_per_node="${TP}" "${REPO_ROOT}/sft_value_type_peft_ddp.py" \
    --dataset_path "${DATASET_PATH}" \
    --model_name "${MODEL}" \
    --output_dir "${ADAPTER_OUT}" \
    --max_length "${MAX_LEN}" \
    --batch_size "${BATCH_SIZE}" \
    --grad_accum "${GRAD_ACCUM}" \
    --epochs "${EPOCHS}" \
    --lr "${LR}" \
    "${BF16_FLAG[@]}" \
    "${QLORA_FLAG[@]}" \
    2>&1 | tee "${LOG_DIR}/sft.log"
fi

if [[ "${MERGE}" == "1" ]]; then
  require_path "${ADAPTER_PATH}" "LoRA adapter"
  python "${REPO_ROOT}/merge_lora_value_type.py" \
    --base "${MODEL}" \
    --adapter "${ADAPTER_PATH}" \
    --out "${MERGED_DIR}" \
    --dtype bf16 \
    2>&1 | tee "${LOG_DIR}/merge.log"
fi

if [[ "${PREDICT}" == "1" ]]; then
  require_path "${MERGED_DIR}" "merged model"
  python "${REPO_ROOT}/generate_fintagging_value_type_predictions.py" \
    --model "${MERGED_DIR}" \
    --dataset-path "${DATASET_PATH}" \
    --split test \
    --output-jsonl "${PRED_DIR}/test_predictions.jsonl" \
    --max-new-tokens "${MAX_NEW_TOKENS}" \
    --bf16 \
    2>&1 | tee "${LOG_DIR}/predict.log"
fi

if [[ "${EVAL}" == "1" ]]; then
  require_path "${PRED_DIR}/test_predictions.jsonl" "prediction file"
  python "${REPO_ROOT}/evaluate_fintagging_context_extraction.py" \
    --gold-dataset "${DATASET_PATH}" \
    --task table \
    --split test \
    --predictions "${PRED_DIR}/test_predictions.jsonl" \
    --output-json "${EVAL_DIR}/test_metrics.json" \
    --per-row-csv "${EVAL_DIR}/test_per_row.csv" \
    2>&1 | tee "${LOG_DIR}/eval.log"
fi

echo
echo "Done. Outputs under: ${MODEL_DIR}"
done
