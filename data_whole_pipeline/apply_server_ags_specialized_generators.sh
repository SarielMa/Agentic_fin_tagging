...
#SBATCH --output=/nfs/roberts/project/pi_sjf37/lm2445/FinAI_tagging_agentic/data_whole_pipeline/%j_ags_task_a_specialized_generators_qwen3_b200.txt
...
export OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/runs_ags_specialized_generators/qwen3_32b}"
...
python -m py_compile \
  "${REPO_ROOT}/ags_specialized_generators.py" \
  "${REPO_ROOT}/run_ags_specialized_generators.py"
python -m unittest \
  "${REPO_ROOT}/test_ags_symbolic_agreement.py" \
  "${REPO_ROOT}/test_ags_specialized_generators.py" \
  "${REPO_ROOT}/test_label_coverage_retriever.py"
python "${REPO_ROOT}/run_ags_specialized_generators.py" "${ARGS[@]}"
