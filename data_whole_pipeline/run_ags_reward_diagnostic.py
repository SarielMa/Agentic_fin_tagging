...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=SCRIPT_DIR / "runs_ags_reward_diagnostic" / "qwen3_32b")
    parser.add_argument("--component-output-dir", type=Path, default=SCRIPT_DIR / "runs_ags_component_validation" / "qwen3_32b")
    parser.add_argument(
        "--sample-path",
        type=Path,
...
def load_component_gate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "adopted_rendering": gate.get("adopted_rendering") or "dual",
        "adopted_initialization": (metrics.get("initialization_recommendation") or {}).get(
            "adopted_initialization",
            "j3_symbolic_select",
        ),
...
def consolidate(rounds: list[dict[str, Any]], top_k: int, rrf_kappa: float) -> list[dict[str, Any]]:
...
def feedback_counts(feedback: dict[str, Any]) -> dict[str, int]:
    return {
        "D_plus": len(feedback.get("supported_dimensions", [])),
        "D_minus": len(feedback.get("contradicted_dimensions", [])),
        "D_question": len(feedback.get("unresolved_dimensions", [])),
    }
...
def load_component_initial_state(
    ...
    rendering = gate["adopted_rendering"]
    initialization = gate["adopted_initialization"]
    ...
    render_records = retrievals_by_render_fact(
        [record for record in retrievals if record["rendering"] == rendering]
    )[rendering]
    ...
    for example in examples:
        ...
        if initialization == "j1_hyp0":
            selected_idx = 0
            initial_rounds = [{"hypothesis": hypotheses_for_fact[0], "candidates": records[0]["candidates"]}]
        elif initialization == "j3_random_one_avg100":
            selected_idx = rng.randrange(len(records))
            initial_rounds = [{"hypothesis": hypotheses_for_fact[selected_idx], "candidates": records[selected_idx]["candidates"]}]
        elif initialization == "j3_union_rrf":
            ...
            initial_rounds = [
                {"hypothesis": hypotheses_for_fact[idx], "candidates": record["candidates"]}
                for idx, record in enumerate(records)
            ]
            initial_rounds.append({"hypothesis": hypotheses_for_fact[selected_idx], "candidates": fused})
        else:
            selected_idx = int(symbolic_choices.get(str(fact_id), symbolic_choices.get(fact_id, 0)))
            initial_rounds = [{"hypothesis": hypotheses_for_fact[selected_idx], "candidates": records[selected_idx]["candidates"]}]
        state[fact_id] = {
...
def run_arm(
    ...
    rng = random.Random(args.seed + (0 if arm == "bandit" else 100000))
    posterior = PosteriorBank(B_OPERATORS, dim=11, ridge=args.posterior_ridge, alpha=args.posterior_alpha, seed=args.seed)
    ...
    for instance_idx, example in enumerate(examples, start=1):
        ...
        for round_idx in range(2, args.rounds + 1):
            before_consolidated = consolidate(episode_rounds, args.top_k, args.rrf_kappa)
            y_before, rank_before, in_before = utility(before_consolidated, example.gold_tags)
            ...
            slate = admissible_slate(feedback)
            if not slate:
            ...
            after_consolidated = consolidate(episode_rounds + [selected_round], args.top_k, args.rrf_kappa)
            y_after, rank_after, in_after = utility(after_consolidated, example.gold_tags)
            delta_y = y_after - y_before

            counterfactual_candidates: list[dict[str, Any]] = []
            counterfactual_delta_y = 0.0
            ...
            if runner_up is not None:
                ...
                replay_consolidated = consolidate(episode_rounds + [replay_round], args.top_k, args.rrf_kappa)
                replay_y, _, _ = utility(replay_consolidated, example.gold_tags)
                counterfactual_delta_y = replay_y - y_before

            delta_replay = delta_y - counterfactual_delta_y
            reward_combined = delta_y + 0.5 * delta_replay
