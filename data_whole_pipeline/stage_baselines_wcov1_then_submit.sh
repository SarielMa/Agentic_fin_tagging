...
RUNS="${REPO}/runs_fintagging_grounding_baseline"
SCRATCH="/nfs/roberts/scratch/pi_sjf37/lm2445/runs_fintagging_grounding_baseline"
LOG="${REPO}/stage_baselines_wcov1.log"
cd "${REPO}"
...
RUNS = Path("/nfs/roberts/project/pi_sjf37/lm2445/FinAI_tagging_agentic/data_whole_pipeline/runs_fintagging_grounding_baseline")
...
JID1=$(sbatch --parsable "${REPO}/apply_server_baselines_wcov1_rerank.sh" direct_retrieval "${OUT_DR}")
say "submitted direct_retrieval rerank: ${JID1}"
JID2=$(sbatch --parsable "${REPO}/apply_server_baselines_wcov1_rerank.sh" one_pass_grounding "${OUT_OP}")
