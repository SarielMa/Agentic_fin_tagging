...
#SBATCH --output=%j_fintag_text_context_sft_b200.txt
...
#
# Example:
#   sbatch --export=ALL,MODE=full apply_server_fintagging_text_context_sft.sh
#   sbatch --export=ALL,MODE=train,MODEL=meta-llama/Llama-3.3-70B-Instruct apply_server_fintagging_text_context_sft.sh
MODE="${MODE:-full}"

FHS_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="${FHS_ROOT}"
PIPELINE_SH="${REPO_ROOT}/attic/run_fintagging_text_context_sft.sh"
...
export DATASET_PATH="${DATASET_PATH:-${REPO_ROOT}/FinTagging_800_200_text_context_sft_arrow}"
export PIPELINE_ROOT="${PIPELINE_ROOT:-${REPO_ROOT}/runs/runs_fintagging_text_context}"
