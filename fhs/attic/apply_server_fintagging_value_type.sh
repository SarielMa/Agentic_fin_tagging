#!/bin/bash
#SBATCH --job-name=fintag_value_type_sft
...
#SBATCH --output=%j_fintag_value_type_sft_b200.txt
...
#   sbatch --export=ALL,MODE=train apply_server_fintagging_value_type.sh
#   sbatch --export=ALL,MODE=train,MODEL=meta-llama/Llama-3.3-70B-Instruct apply_server_fintagging_value_type.sh
#   sbatch --export=ALL,MODE=train,MODELS="Qwen/Qwen2.5-14B-Instruct meta-llama/Llama-3.3-70B-Instruct" apply_server_fintagging_value_type.sh
MODE="${MODE:-full}"

FHS_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="${FHS_ROOT}"
PIPELINE_SH="${REPO_ROOT}/attic/run_fintagging_value_type_sft.sh"
...
# Defaults for this task. You can override any of these with sbatch --export.
export MODELS="${MODELS:-Qwen/Qwen2.5-14B-Instruct meta-llama/Llama-3.3-70B-Instruct}"
if [[ -n "${MODEL:-}" ]]; then
  export MODEL
fi
export DATASET_PATH="${DATASET_PATH:-${REPO_ROOT}/FinTagging_800_200_value_type_sft_arrow}"
export PIPELINE_ROOT="${PIPELINE_ROOT:-${REPO_ROOT}/runs/runs_fintagging_value_type}"
