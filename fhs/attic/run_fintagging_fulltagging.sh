...
RUNS_ROOT="${RUNS_ROOT:-${REPO_ROOT}/runs/runs_fintagging_fulltagging}"
...
TEXT_EXTRACTOR_MODEL="${TEXT_EXTRACTOR_MODEL:-${REPO_ROOT}/runs/runs_fintagging_text_context/qwen2.5_14b_instruct/sft_3ep/merged}"
TABLE_EXTRACTOR_MODEL="${TABLE_EXTRACTOR_MODEL:-${REPO_ROOT}/runs/runs_fintagging_table_context/qwen2.5_14b_instruct/sft_3ep/merged}"
EXTRACTION_PREDICTIONS="${EXTRACTION_PREDICTIONS:-${OUTPUT_DIR}/extraction_predictions.jsonl}"
TEXT_EXTRACTION_PREDICTIONS="${TEXT_EXTRACTION_PREDICTIONS:-${REPO_ROOT}/runs/runs_fintagging_text_context/qwen2.5_14b_instruct/sft_3ep/predictions/test_predictions.jsonl}"
TABLE_EXTRACTION_PREDICTIONS="${TABLE_EXTRACTION_PREDICTIONS:-${REPO_ROOT}/runs/runs_fintagging_table_context/qwen2.5_14b_instruct/sft_3ep/predictions/test_predictions.jsonl}"
...
require_path "${REPO_ROOT}/attic/build_fintagging_fulltagging_grounding_input.py" "fullTagging input builder"
require_path "${REPO_ROOT}/attic/evaluate_fintagging_fulltagging_pipeline.py" "fullTagging evaluator"
require_path "${REPO_ROOT}/scripts/run_fintagging_grounding_baseline.sh" "grounding pipeline"
require_path "${TAXONOMY_JSONL}" "enriched retrieval taxonomy"
mkdir -p "${OUTPUT_DIR}"
mkdir -p "${HF_HOME}" "${HF_DATASETS_CACHE}" "${HF_HUB_CACHE}" "${TRANSFORMERS_CACHE}" "${VLLM_CACHE_ROOT}" "${TORCH_HOME}"
...
python -m py_compile "${REPO_ROOT}/attic/generate_fintagging_fulltagging_extractions.py"
python -m py_compile "${REPO_ROOT}/attic/build_fintagging_fulltagging_grounding_input.py"
python -m py_compile "${REPO_ROOT}/attic/evaluate_fintagging_fulltagging_pipeline.py"
...
if [[ "${RUN_EXTRACTION}" == "1" ]]; then
  ...
  if [[ "${EXTRACTION_BACKEND}" == "vllm" ]]; then
    python "${REPO_ROOT}/attic/generate_fintagging_fulltagging_extractions.py" \
      "${EXTRACTION_COMMON_ARGS[@]}" \
  ...
  python "${REPO_ROOT}/attic/build_fintagging_fulltagging_grounding_input.py" \
    --original-test-parquet "${ORIGINAL_TEST_PARQUET}" \
...
if [[ "${RUN_GROUNDING}" == "1" ]]; then
  bash "${REPO_ROOT}/scripts/run_fintagging_grounding_baseline.sh"
  ...
  python "${REPO_ROOT}/attic/evaluate_fintagging_fulltagging_pipeline.py" "${FULLTAGGING_EVAL_ARGS[@]}"
fi
