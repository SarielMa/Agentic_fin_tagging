...
VERDICTS="${RUN}/verdicts_k10_hint6/llm_verifier_verdicts.json"
ARM=no_determ
OUT="${RUN}/rerank_no_determ_k10hint6"
...
if [[ ! -f "${RANKING}" ]]; then
  echo "[$(date +%H:%M:%S)] dumping hint6 arm (all 6 dims supplied as evidence) (mode=llm_drop) against verdicts_k10_hint6"
