...
VERDICTS="${RUN}/verdicts_k10_hint/llm_verifier_verdicts.json"
ARM=no_determ
OUT="${RUN}/rerank_no_determ_k10hint"
...
if [[ ! -f "${RANKING}" ]]; then
  echo "[$(date +%H:%M:%S)] dumping hint arm (mode=llm_drop) against verdicts_k10_hint"
